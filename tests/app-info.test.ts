...
describe("app info", () => {
  it("builds the API index response", () => {
    ...
    expect(response).toEqual({
      ...
      status: "api-ready",
      phase: CURRENT_PHASE,
      capabilities: ["event ingestion", "block sealing", "Merkle proofs", "Ed25519 verification"],
      routes: [
        { method: "GET", path: "/", description: "API index" },
        { method: "GET", path: "/health", description: "Health check" },
        { method: "POST", path: "/events", description: "Ingest an event" },
        { method: "GET", path: "/events/:event_id", description: "Fetch a stored event" },
        { method: "POST", path: "/blocks/create", description: "Seal pending events into a block" },
        { method: "GET", path: "/blocks", description: "List sealed blocks" },
        { method: "GET", path: "/proof/:event_id", description: "Fetch a proof envelope" },
        { method: "POST", path: "/verify", description: "Verify a proof envelope" },
        { method: "GET", path: "/keys/current", description: "Fetch the current verification key" }
      ],
      docs: {
        api: "docs/API.md",

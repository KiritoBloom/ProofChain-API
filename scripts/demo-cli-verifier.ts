...
async function main(): Promise<void> {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "proofchain-cli-"));
  ...
  const result = await import("node:child_process").then(
    ({ execFileSync }) =>
      execFileSync("node", ["./node_modules/tsx/dist/cli.mjs", "scripts/verify-proof.ts", "verify", proofFilePath], {
        cwd: process.cwd(),
        encoding: "utf8"
      })
  );

  console.log("Generated proof file");
  console.log(await readFile(proofFilePath, "utf8"));
  console.log("");

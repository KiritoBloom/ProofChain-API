...
const persistenceContexts = new Map<string, Promise<ProofChainPersistenceContext>>();

export async function getProofChainPersistenceContext(env: AppEnv): Promise<ProofChainPersistenceContext> {
  ...
  const contextPromise = createProofChainPersistenceContext(env).catch((error: unknown) => {
...
async function createProofChainPersistenceContext(env: AppEnv): Promise<ProofChainPersistenceContext> {

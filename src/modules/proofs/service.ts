...
import type { BlockRepository, EventRepository } from "../../types/persistence.js";

export function createGetProofByEventIdService(dependencies: {
  blockRepository: BlockRepository;
  eventRepository: EventRepository;
}) {
  return async function getProofByEventId(eventId: string): Promise<ProofEnvelope> {
    ...
    const blockRecord = await dependencies.blockRepository.getBlockById(eventRecord.block_id);

    if (!blockRecord) {
      throw new NotFoundError(`Block not found for event: ${eventId}`);
    }

    const orderedEvents = await dependencies.eventRepository.getEventsByBlockId(blockRecord.block_id);
    const leafIndex = orderedEvents.findIndex((event) => event.event_id === eventRecord.event_id);

    if (leafIndex === -1) {
      throw new NotFoundError(`Sealed event was not found in its block ordering: ${eventId}`);
    }
...
export function createVerifyProofService() {
  return async function verifyProof(input: ProofEnvelope & { public_key: string }): Promise<{ valid: boolean }> {
